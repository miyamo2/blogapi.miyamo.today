		...
		"happy_path/with-cursor/single/has-previous", func() {
			...
			WhenDouble(
				queries.ListBeforeWithLimitAndCursor(
					AnyContext(),
					Equal(rdb.ListBeforeWithLimitAndCursorParams{ID: "0", Limit: 2}),
				),
			).
				ThenReturn(
					[]rdb.ListBeforeWithLimitAndCursorRow{
						{
							ID:        "2",
							Name:      "tag2",
							CreatedAt: synchro.New[tz.UTC](2020, 1, 1, 0, 0, 0, 0),
	...
	s.Run(
		"happy_path/with-cursor/multiple/has-previous", func() {
			...
			WhenDouble(
				queries.ListBeforeWithLimitAndCursor(
					AnyContext(),
					Equal(rdb.ListBeforeWithLimitAndCursorParams{ID: "0", Limit: 3}),
				),
			).
				ThenReturn(
					[]rdb.ListBeforeWithLimitAndCursorRow{
