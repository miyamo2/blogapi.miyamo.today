...
import (
	...
	mdb "github.com/miyamo2/api.miyamo.today/article-service/internal/mock/api.miyamo.today/core/db"
	mquery "github.com/miyamo2/api.miyamo.today/article-service/internal/mock/app/usecase/query"
...
func TestGetPrev_Execute(t *testing.T) {
	...
	tests := map[string]testCase{
		"happy_path": {
			...
			setupArticleService: func(qs *mquery.MockArticleService[query.Tag, *query.Article], stmt *mdb.MockStatement) {
				qs.EXPECT().GetAll(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
			...
			want: func() want {
				o := dto.NewGetPrevOutDto([]dto.Article{
					dto.NewArticle(
						"1",
						"happy_path",
						"## happy_path",
		...
		"happy_path/zero_article": {
			...
			want: func() want {
				o := dto.NewGetPrevOutDto(make([]dto.Article, 0), false)
				return want{out: &o, err: nil}
			}(),
		},
		...
		"unhappy_path/transaction_managers_returns_error": {
			...
			setupArticleService: func(qs *mquery.MockArticleService[query.Tag, *query.Article], stmt *mdb.MockStatement) {
				qs.EXPECT().GetAll(gomock.Any(), gomock.Any(), gomock.Any()).Times(0)
			},
			want: func() want {
