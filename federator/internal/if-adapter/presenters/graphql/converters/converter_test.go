...
func TestConverter_ToUpdateArticle(t *testing.T) {
	type args struct {
		ctx  context.Context
		from dto.UpdateArticleTitleOutDTO
	}
	type want struct {
	...
	tests := map[string]testCase{
		"happy_path": {
			...
			args: args{
				ctx:  context.Background(),
				from: dto.NewUpdateArticleTitleOutDTO("event_id", "article_id", "client_mutation_id"),
			},
			want: want{
				out: &model.UpdateArticleTitlePayload{
					ArticleID: "article_id",
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			c := tt.sut()
			got, err := c.ToUpdateArticleTitle(tt.args.ctx, tt.args.from)
			if !errors.Is(err, tt.want.err) {
				t.Errorf("ToUpdateArticleTitle() error = %v, want %v", err, tt.want.err)
				return
			}
			if diff := cmp.Diff(got, tt.want.out, cmpOpts...); diff != "" {
				t.Error(diff)
...
func TestConverter_ToUpdateArticleBody(t *testing.T) {
	type args struct {
		ctx  context.Context
		from dto.UpdateArticleBodyOutDTO
	}
	type want struct {
	...
	tests := map[string]testCase{
		"happy_path": {
			...
			args: args{
				ctx:  context.Background(),
				from: dto.NewUpdateArticleBodyOutDTO("event_id", "article_id", "client_mutation_id"),
			},
			want: want{
				out: &model.UpdateArticleBodyPayload{
					ArticleID: "article_id",
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			c := tt.sut()
			got, err := c.ToUpdateArticleBody(tt.args.ctx, tt.args.from)
			if !errors.Is(err, tt.want.err) {
				t.Errorf("ToUpdateArticleBody() error = %v, want %v", err, tt.want.err)
				return
			}
			if diff := cmp.Diff(got, tt.want.out, cmpOpts...); diff != "" {
				t.Error(diff)
...
func TestConverter_ToDetachTags(t *testing.T) {
	type args struct {
		ctx  context.Context
		from dto.DetachTagsOutDTO
	}
	type want struct {
	...
	tests := map[string]testCase{
		"happy_path": {
			...
			args: args{
				ctx:  context.Background(),
				from: dto.NewDetachTagsOutDTO("event_id", "article_id", "client_mutation_id"),
			},
			want: want{
				out: &model.DetachTagsPayload{
					ArticleID: "article_id",
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			c := tt.sut()
			got, err := c.ToDetachTags(tt.args.ctx, tt.args.from)
			if !errors.Is(err, tt.want.err) {
				t.Errorf("ToDetachTags() error = %v, want %v", err, tt.want.err)
				return
			}
			if diff := cmp.Diff(got, tt.want.out, cmpOpts...); diff != "" {
				t.Error(diff)
